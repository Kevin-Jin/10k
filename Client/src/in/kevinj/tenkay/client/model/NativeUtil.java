package in.kevinj.tenkay.client.model;
...
public class NativeUtil {
	...
	private static String getProcessForWindow(HWND hwnd) {
		...
		String processPath = Native.toString(buffer);
...
System.out.println(f.length() + " " + IoAndEncodingUtil.toBase64Url(md5));
		} catch (IOException e) {
			Logger.getLogger(NativeUtil.class.getName()).log(Level.INFO, "", e);
	 ...
	 */
	public static Runnable startListeningForChanges(final Model m) {
							...
							case User32DLL.EVENT_SYSTEM_MINIMIZEEND_INT:
								...
								if (nowProcess != currentProcess) {
									...
									SwingUtilities.invokeLater(new Runnable() {
										@Override
										public void run() {
											m.setProcess(getProcessForWindow(hWnd));
										}
									});
								}
								break;
